...
def test_TC_SEARCH_01_open_tests_page(tests_page):
...
@pytest.mark.parametrize("key", ["TOEIC"])
def test_TC_SEARCH_02_search_with_certificate_keyword(tests_page, key):    
    tests_page.search(key)
    for result in tests_page.get_results():
        assert key.upper() in result.text.upper()

@pytest.mark.parametrize("key", ["New Economy"])
def test_TC_SEARCH_03_search_with_thematic_keyword(tests_page, key):    
    tests_page.search(key)
    for result in tests_page.get_results():
        assert key.upper() in result.text.upper()

@pytest.mark.parametrize("invalid_key", ["abcxyz123"])
def test_TC_SEARCH_04_search_with_invalid_keyword(tests_page, invalid_key):
    tests_page.search(invalid_key)
    assert tests_page.is_no_result_displayed() or len(tests_page.get_results()) == 0

@pytest.mark.parametrize("invalid_key", ["!@#$%^&*()"])
def test_TC_SEARCH_05_search_with_special_characters(tests_page, invalid_key):
    tests_page.search(invalid_key)
...
def test_TC_SEARCH_06_search_with_empty_keyword(tests_page):
    tests_page.search("")
    assert len(tests_page.get_results()) > 0

@pytest.mark.parametrize("subject", ["IELTS Academic"])
def test_TC_FILTER_01_filter_by_subject(tests_page, subject):
    tests_page.filter(subject)
    assert tests_page.check_filter_results(subject)

def test_TC_FILTER_02_filter_mini_tests(tests_page):
...
def test_TC_SEARCH_AND_FILTER_01_search_and_filter_combination(tests_page, subject, keyword):
    tests_page.filter(subject)
...
def test_TC_PAGING_01_pagination(tests_page, page_number):
